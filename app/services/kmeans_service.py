...
* Pelabelan kategori ketimpangan (Rendah/Tinggi) berdasarkan rata-rata centroid.
"""
...
def label_clusters_by_centroid(
    ...
    n_clusters: int,
    label_pool: Sequence[str] = KATEGORI_LABELS,
) -> dict[int, str]:
    ...
    * Mapping ke kategori ketimpangan ``Rendah`` dan ``Tinggi``.
    * Jika jumlah cluster > 2, urutan centroid dibagi ke dua kategori
    ...
    if n_clusters == 1:
        mapping[int(order[0])] = "Rendah"
    elif n_clusters == 2:
        mapping[int(order[0])] = "Rendah"
        mapping[int(order[1])] = "Tinggi"
    else:
